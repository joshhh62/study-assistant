...
- Structured-output pipeline: the LLM's response is schema-validated and retried on failure, never trusted blindly — see Handling bad AI output below
...
- Environment variable: VITE_API_BASE_URL = the Render URL from the backend step (no trailing slash).
